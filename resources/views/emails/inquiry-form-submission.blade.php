...
<head>
    ...
    <title>New Inquiry — Precious Real Estate</title>
    <style>
    ...
    <div class="email-wrapper">
        <div class="container">
            ...
            <div class="header">
                ...
                <p>New inquiry received via the website</p>
            </div>

            <div class="content">
                <span class="badge">New Inquiry</span>
                <h2>{{ $data['service'] ?? 'General' }} Inquiry</h2>
                ...
                <div class="section">
                    ...
                    <table class="grid" role="presentation" cellpadding="0" cellspacing="0">
                        ...
                        <tr>
                            ...
                            <td>
                                <p class="field-label">Preferred Contact</p>
                                <p class="field-value">{{ $data['contactMethod'] ?? 'N/A' }}</p>
                            </td>
                        </tr>
                        @if (!empty($data['location']) || !empty($data['property_id']))
                            <tr>
                                @if (!empty($data['location']))
                                    <td>
                                        <p class="field-label">Location</p>
                                        <p class="field-value">{{ $data['location'] }}</p>
                                    </td>
                                @endif
                                @if (!empty($data['property_id']))
                                    <td>
                                        <p class="field-label">Property ID</p>
                                        <p class="field-value">#{{ $data['property_id'] }}</p>
                                    </td>
                                @endif
                            </tr>
                        @endif
                ...
                @if (!empty($data['serviceFields']) && count(array_filter($data['serviceFields'])))
                    <hr class="divider">
                    <div class="section">
                        <p class="section-label">{{ $data['service'] ?? 'Service' }} Details</p>
                        <table class="grid" role="presentation" cellpadding="0" cellspacing="0">
                            @php $pairs = array_chunk(array_filter($data['serviceFields']), 2, true); @endphp
                            @foreach ($pairs as $pair)
                                <tr>
                                    @foreach ($pair as $key => $value)
                                        <td>
                                            <p class="field-label">{{ \Illuminate\Support\Str::headline($key) }}</p>
                ...
                @if (!empty($data['additionalDetails']))
                    ...
                    <div class="section">
                        ...
                        <div class="message-box">{{ $data['additionalDetails'] }}</div>
                    </div>
                @endif
