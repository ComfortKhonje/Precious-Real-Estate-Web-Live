@props([
    ...
    'image' => asset('brand-assets/1 Home Page/CTA Image.png'),
    'mobileImage' => asset('brand-assets/1 Home Page/CTA Image mobile.png'),
    'showContactInfo' => true
])

<section class="relative pt-8 pb-0">
            ...
            <div class="absolute inset-y-0 left-[35%] -translate-x-1/2 w-[40%] bg-gradient-to-l from-[#202020] to-transparent z-0"></div>
                ...
                <div class="p-6 md:p-16 text-white relative z-10 flex flex-col justify-between">
                    <div class="gap-2 md:gap-4">
                        <div class="badge-yellow w-fit">{{ $badge }}</div>
                        <h2 class="text-4xl md:text-5xl font-heading mb-2 md:mb-6">{{ $title }}</h2>
                        <p class="text-gray-300 text-lg mb-10 max-w-lg">
                            {{ $description }}
                        </p>
                    </div>
                    <div class="flex flex-wrap gap-4">
                        <a href="{{ route('inquiry') }}" class="btn-secondary tracking-wider w-full md:w-fit font-semibold hover:scale-[1.03] active:scale-[0.97] transition-all duration-300">REQUEST CONSULTATION</a>
                ...
                <div class="relative h-full w-full lg:h-full overflow-hidden">
                        {{-- Desktop Image --}}
                        <img loading="lazy" decoding="async" src="{{ $image }}" alt="CTA Image" class="hidden lg:block h-full w-full object-cover object-center">
                        {{-- Mobile Image --}}
                        <img loading="lazy" decoding="async" src="{{ $mobileImage }}" alt="CTA Image Mobile" class="block lg:hidden h-full w-full object-cover object-center">
                </div>
            </div>
        </div>
    </div>
