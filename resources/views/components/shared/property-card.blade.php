...
<div class="rounded-[1.5rem] md:rounded-[2rem] overflow-hidden shadow-sm border border-gray-100 flex flex-col hover:shadow-xl transition duration-300 p-2 gap-2 h-fit group">
    {{-- Image Container --}}
    ...
    <div class="flex-1 flex flex-col px-2 pb-2">
        <div class="flex flex-col mb-4">
            ...
            <h3 class="text-xl font-bold text-brand-black mb-1">{{ $price }}</h3>
            <p class="text-gray-500 text-xs line-clamp-2 leading-relaxed h-8">
                {{ $description ?: $title }}
            </p>
        </div>

        <a href="{{ route('property.view', $slug) }}" class="mt-auto bg-brand-black text-white text-center rounded-full py-3 px-6 font-bold text-[10px] uppercase tracking-[0.2em] hover:bg-gray-800 transition duration-300">
            VIEW DETAILS
        </a>
    </div>
</div>
